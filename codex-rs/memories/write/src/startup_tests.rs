...
#[tokio::test]
async fn memories_configured_models_override_provider_defaults() -> anyhow::Result<()> {
    let server = start_mock_server().await;
    let home = Arc::new(TempDir::new()?);
    let test = build_test_codex(&server, home).await?;
    ...
    config.memories.extract_model = Some("custom-extract-model".to_string());
    config.memories.consolidation_model = Some("custom-consolidation-model".to_string());
    let provider = create_model_provider(
    ...
    assert_eq!(
        crate::phase1::extract_model(&config, provider.as_ref()),
        "custom-extract-model"
    );
    assert_eq!(
        crate::phase2::consolidation_model(&config, provider.as_ref()),
        "custom-consolidation-model"
    );
